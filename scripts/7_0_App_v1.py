...
def update_graph(cultivars_selectionnes, variable_x, variable_y, type_visualisation):
    ...
    if type_visualisation == 'facet_grid':
        # Créer une grille de graphiques pour chaque année de la colonne 'year'
        ...
        for i, facet in enumerate(unique_facets):
            ...
            fig.update_xaxes(title_text=variable_x.capitalize(), row=row, col=col, tickvals=list(
                range(1, 13)) if variable_x == 'age_plan' else None, range=[1, 12], autorange=True)
            fig.update_yaxes(title_text=variable_y.capitalize(),
                             row=row, col=col, autorange=True)
        ...
        if type_visualisation == 'box_scatter':
            ...
            fig = px.box(
                ...
                notched=True,  # Activer l'option boxnotch
                # Ajouter unique_id dans le hover
    ...
    # Ajouter des lignes verticales pour 'age_plan' seulement
